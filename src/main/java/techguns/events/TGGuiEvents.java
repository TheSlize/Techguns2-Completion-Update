...
public class TGGuiEvents extends Gui {
    ...
    private static void updateHudScaling(Minecraft mc) {
        ...
        AMMO_TEXT_SCALE = TGConfig.clientside.ammoHud.cl_ammoTextScale * scaleFactor;
        AMMO_ICON_SCALE = TGConfig.clientside.ammoHud.cl_ammoIconScale * scaleFactor;
        AMMO_MAG_TEXT_SCALE = TGConfig.clientside.ammoHud.cl_ammoMagTextScale * scaleFactor;
        HUD_MARGIN_RIGHT = (int) (TGConfig.clientside.ammoHud.cl_hudMarginRight * scaleFactor);
        HUD_MARGIN_BOTTOM = (int) (TGConfig.clientside.ammoHud.cl_hudMarginBottom * scaleFactor);
        ICON_TEXT_GAP = (int) (TGConfig.clientside.ammoHud.cl_iconTextGap * scaleFactor);
        TEXT_MAG_GAP = (int) (TGConfig.clientside.ammoHud.cl_textMagGap * scaleFactor);
    }
    ...
    public void onRenderGameOverlyEvent(RenderGameOverlayEvent event) {
        ...
        if (!itemOff.isEmpty() && itemOff.getItem() instanceof GenericGun gun) {
            if (GunManager.canUseOffhand(item, itemOff)) {
                if (TGConfig.clientside.ammoHud.cl_enableLegacyHud)
                    this.drawGunAmmoCount(mc, sr, gun, itemOff, ply, props, -12);
