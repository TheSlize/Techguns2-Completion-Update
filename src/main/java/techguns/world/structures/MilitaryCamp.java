...
//		structures.add(new FactoryHouseSmall(9, 6, 10, 16, 6, 16).setSwapXZ(true));
    ...
    public void init(int posX, int posY, int posZ, int sizeX, int sizeZ) {
        ...
        campRadiusSquared = Vec2.substract(new Vec2(this.posX, this.posZ), new Vec2((float) this.posX + ((float) this.sizeX / 2.0f), (float) this.posZ + ((float) this.sizeZ / 2.0f))).lenSquared();
    ...
    class CampSegment {
        ...
        private WorldgenStructure getMatchingStructure(ArrayList<WorldgenStructure> list) {
            ...
            for (WorldgenStructure str : list) {
                if (str.canSwapXZ) {
                    if (((sizeX >= str.minX && sizeZ >= str.minZ) || (sizeZ >= str.minX && sizeX >= str.minZ)) && (((str.maxX == -1 || sizeX <= str.maxX) && (str.maxZ == -1 || sizeZ <= str.maxZ)) || ((str.maxX == -1 || sizeZ <= str.maxX) && (str.maxZ == -1 || sizeX <= str.maxZ)))) {
                        matchingStructures.add(str);
                    }
                } else {
                    if (sizeX >= str.minX && sizeZ >= str.minZ && (str.maxX == -1 || sizeX <= str.maxX) && (str.maxZ == -1 || sizeZ <= str.maxZ)) {
                        matchingStructures.add(str);
                    }
                }
            }
